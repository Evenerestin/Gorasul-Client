...
export default function Ornament() {
  ...
  const saveOrnament = async (data: string | null) => {
    ...
    try {
      const response = await api.post('/save-ornament', { ornamentData: data });
        ...
        >
          ...
          >
            ...
            {activeModal === 'save-success' && (
              <>
                <p className="mb-4">
                  Bombka została zapisana pomyślnie. Możesz ją teraz znaleźć na gorasuloince!
                </p>
            ...
            {activeModal === 'save-error' && (
              <p>Niestety nie udało się zapisać bombki, proszę spróbować ponownie później.</p>
            )}

            {activeModal === 'clear-file' && (
              <>
                <p className="mb-4">Jesteś pewien, że chcesz wyczyścić całą bombkę?</p>
                <div className="flex justify-between mt-6">
                  <button
                    onClick={() => setActiveModal(null)}
                    className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700"
